...
@torch.no_grad()
def run(poseweights="yolov7-w6-pose.pt",source="./input/barbell_shrugs_new.mp4",device='cpu',curltracker=False,drawskeleton=False):
    path = source
    ...
    if ext in ["mp4","webm","avi"] or ext not in ["mp4","webm","avi"] and ext.isnumeric():
        ...
        cap = cv2.VideoCapture("./input/barbell_shrugs_new.mp4")  
        webcam = False
        ...
        np.savetxt("D:/DFKI Berlin/human_pose_estimation/angles_bicep.csv", angles, encoding="utf-8")
        np.savetxt("D:/DFKI Berlin/human_pose_estimation/percentages_bicep.csv", percentages, encoding="utf-8")
        np.savetxt("D:/DFKI Berlin/human_pose_estimation/bars_bicep.csv", bars, encoding="utf-8")
        print(angles)
        print(percentages)
        print(bars)

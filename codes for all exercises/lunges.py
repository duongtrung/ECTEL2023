...
@torch.no_grad()
def run(poseweights="yolov7-w6-pose.pt",source="./input/lunges_new.mp4",device='cpu',curltracker=False,drawskeleton=False):
    path = source
    ...
    if ext in ["mp4","webm","avi"] or ext not in ["mp4","webm","avi"] and ext.isnumeric():
        ...
        cap = cv2.VideoCapture("./input/lunges_new.mp4")  
        webcam = False
            ...
            if ret: 
                ...
                if curltracker:
                    ...
                    for idx in range(output.shape[0]):
                        ...
                        angleR = findAngle(img,kpts,11,13,15,draw = True)
                        angleL = findAngle(img,kpts,12,14,16,draw =True)
                        ...
                        Percentage = np.interp(angleR, (end_angle_R, start_angle_R), ( 100, 0))
                        #Percentage = (PercentageR + PercentageL) / 2 
                        #print("Percentage: {:.2f}%".format(Percentage))
                        percentages.append(Percentage)

                        #barR = np.interp(angleR, (start_angle_R, end_angle_R), (int(fh) - 100, 100))
                        bar = np.interp(angleR, (end_angle_R, start_angle_R), (100, int(fh) - 100))
                        #bar = (barR + barL) / 2

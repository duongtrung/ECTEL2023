...
@torch.no_grad()
def run(poseweights="yolov7-w6-pose.pt",source="./input/lateral_pulls_new.mp4",device='cpu',curltracker=False,drawskeleton=False):
    path = source
    ...
    if ext in ["mp4","webm","avi"] or ext not in ["mp4","webm","avi"] and ext.isnumeric():
        ...
        cap = cv2.VideoCapture("./input/lateral_pulls_new.mp4")  
        webcam = False

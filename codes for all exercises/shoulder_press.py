...
@torch.no_grad()
def run(poseweights="yolov7-w6-pose.pt",source="./input/shoulder_press.mp4",device='cpu',curltracker=False,drawskeleton=False):
    path = source
    ...
    if ext in ["mp4","webm","avi"] or ext not in ["mp4","webm","avi"] and ext.isnumeric():
        ...
        cap = cv2.VideoCapture("./input/shoulder_press.mp4")  
        webcam = False
            ...
            if ret: 
                ...
                if curltracker:
                    ...
                    for idx in range(output.shape[0]):
                        ...
                        #Percentage = np.interp(angleL, (start_angle_L, end_angle_L), ( 100, 0))
                        ...
                        #bar = np.interp(angleL, (start_angle_L, end_angle_L), (int(fh) - 100, 100))
                        bars.append(barR)

                        data.append([angleR, Percentage, barR])
        ...
        np.savetxt("D:/DFKI Berlin/human_pose_estimation/angles_shoulder_press.csv", angles, encoding="utf-8")
        np.savetxt("D:/DFKI Berlin/human_pose_estimation/percentages_shoulder_press.csv", percentages, encoding="utf-8")
        np.savetxt("D:/DFKI Berlin/human_pose_estimation/bars_bicep_shoulder_press.csv", bars, encoding="utf-8")
        print(angles)
        print(percentages)
        print(bars)
